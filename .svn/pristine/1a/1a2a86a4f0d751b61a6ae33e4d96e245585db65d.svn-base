  ...
  ajax({
    ...
    cb: function (data) {
      ...
      $('#charge').on('click',function(){
            ...
            window.location.href = data.bank_size ==0?'account_chargeAmount.html':'account_charge.html?type=1';
      })
...
app.method.addSubbranch = function (opts) {
  ...
  if (wrap.length > 0) {
    wrap.find('>.header>span').text(wrap.data('type') == '1' ? '开户分行' : '开户支行');
  }
  else {
    wrap = $(wrapStr);
    $(document.body).append(wrap);
    ...
    function getData() {
      ...
      console.log($('.js_bank_name_quick').text())
      ajax({
        ...
        data: {
          ...
          city:$('.js-citiy').val(),
          bankName:$('#js_bank_name_quick').text()
        },
        cb: function (data) {
          console.log(data);
          errDiv.text('');
