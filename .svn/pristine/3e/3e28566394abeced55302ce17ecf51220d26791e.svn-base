...
;(function(factory) {
	...
	$.fn.mobileAreaSelect = function( options ) {
		...
		var settings = $.extend({
			...
			data:GETPROVINCECITY_JSON_URL,//数据路径
			index:0,
			value:[],//地址code
			datas:{version: app_data.version,
			...
			bindEvent:function(){
				...
				this.$ms.on("click",function(){
					...
					_this.scroller.children().on('touchend', function(e) {
						...
						_this.value[i] = $(dl.children().get(index)).attr('parentId');
...
//				return str;
			...
			format: function() {
				var _this = this;
				$(_this.data).each(function(idx, item) {  
				     item.code=item.pcCode;
				     item.name=item.pcName;
				});
				this.f(this.data);
			},
			f: function(data) {
				...
				var str = '<dl><dd ref="0" parentId="0">——</dd>';
				var focus = 0,
					childData, top = _this.mtop;
				if (_this.index !== 0 && _this.value[_this.index - 1] == "0") {
					str = '<dl><dd ref="0" parentId="0" class="focus">——</dd>';
					...
					for (var j = 0, len = item.length; j < len; j++) {
						...
						str += '<dd class="' + cls + '" ref="' + id + '" parentId="' + id + '"  pcType="' + pcType + '" >' + item[j].name + '</dd>';
				...
				// 	async: false,
				// 	url: settings.data,
				// 	data:{version:app_data.version,tokenid:getCookie('tokenid'),parentId:id,pcType:pcType},
				// 	type: 'GET',
				// 	success: function(result) {
