...
var tokenid=getCookie('tokenid')?getCookie('tokenid'):getQueryString('tokenid');
    ...
    $.ajax({//标的详情
        ...
        success:function(data){
            if(data.code=='000'){
                ...
                data.incomereport = data.incomereport == '1' ? '有' : '无';//收入证明
                data.creditreport = data.creditreport == '1' ? '有' : '无';//信用报告
                ...
                data.household_register_cd = ['', '有房无贷款', '有房有贷款', '无', '按揭自置', '无按揭自置'][data.household_register_cd] || '未评估';//房产
                ...
                $('.assess').click(function(){
                        tools.dialog({
                            content:'评估结果仅供参考，不作为出借依据，出借人应独立决策并自行承担出借中存在的风险，出借须谨慎',
                            btn:['确认'],
                            cb:[undefined]
                        })
                    })
